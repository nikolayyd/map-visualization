...
function AzureMap() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      const loadedPoints = JSON.parse(stored).map((coords) => new atlas.data.Point(coords));
  ...
  useEffect(() => {
    ...
    const clickHandler = (e) => {
      ...
      setPoints(prev => {
        ...
        localStorage.setItem("points", JSON.stringify(updatedPoints.map(p => p.coordinates)));
  ...
  const updateRoute = (pointsArray) => {
    ...
    routeSourceRef.current.add(new atlas.data.Feature(new atlas.data.LineString(coordinates1)));
    routeSourceRef.current.add(new atlas.data.Feature(new atlas.data.LineString(coordinates2)));
    routeSourceRef.current.add(new atlas.data.Feature(new atlas.data.LineString(coordinates3)));
    routeSourceRef.current.add(new atlas.data.Feature(new atlas.data.LineString(coordinates4)));
  };

  return (
    <div>
      <button onClick={() => setEnabledAdding(true)}>Добави точка</button>
